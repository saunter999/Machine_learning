...
class LogRegress:
  ...
  def __init__(self,X,Y,C=0.0,batch_size=1,nfflag=True,costflag=True):
    ...
    print("number of features=",self.nfeature)
  ...
  def training(self,Nit,tol,alpha,method):
      ...
      if method=='SGD':
         self.theta=self.SGD(self.theta,self.sgd_grad_eval,mini_param,self.batch_size)

      if method=='BFGS':
         res=minimize(self.costfunction,(self.theta).reshape(-1,),method='BFGS',jac=self.grad_eval,\
	 tol=tol,options={'disp':True,'maxiter':Nit})
	 ...
	 jac=self.grad_eval,tol=tol,options={'disp':True,'maxiter':Nit}) 
	 self.theta=res.x.reshape(-1,1)
	 print("Optimization status",res.message)
	 print("# of iterations=",res.nit)
      ...
      print("batch_size(only used in SGD)=",self.batch_size)
      print("Maximum_iters/epochs =",Nit)
      print("tolerance=",tol)
      print("learning rate for SGD=",alpha)
  ...
  def sgd_grad_eval(self,theta,ib,bsize):
